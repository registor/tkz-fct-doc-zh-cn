...
\item 版本修订：
  \begin{itemize}
   ...
   %  \parg{$x_a..x_b$}
   \item 现在，在\TIKZ{}中使用定义域，而不再使用\parg{$x_a..x_b$}的表示形式
...
\item 当函数表达式作为一个参数传递给\tkzcname{tkzFct}命令时，
	...
	\tkzcname{tkzFctgnu}命令使用\enquote{a}实现对该函数的引用，
	在同一\tkzname{tikzpicture}环境中，也可以使用\enquote{b}\dots{}
	这样的方式按顺序引用定义的函数。
...
还有一种可能是xxx.gnuplot文件错误，此时，只需用文本编辑器打开它，就可以编辑其\tkzname{gnuplot}命令。
值得注意的是：\tkzname{gnuplot}在4.2版后，其语法发生了变化(4.4或4.5后会推出新语法)。
若使用旧版本，其创建表格命令是：\tkzname{set table}。
   ...
   \item 在\tkzname{gnuplot}定义了\tkzname{pi}用于表示$\pi$
   \item 在\tkzname{fp.sty}定义了\tkzcname{FPpi}用于表示$\pi$。
