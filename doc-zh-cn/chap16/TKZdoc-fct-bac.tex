...
\section{在\tkzname{alterqcm}和\tkzname{tkz-tab}宏包中使用\tkzname{tkz-fct}宏包绘制函数曲线}
...
图1是函数$f$在$\mathbf{R}^+$域中的曲线，图2是基元函数$f$在$\mathbf{R}^+$域中的曲线。
...
已知$\mathbf{R}^+$定义域内的严格正函数$k$的变分表为：
...
% orthonormal  $O;\vec{\imath};\vec{\jmath}$.
	...
	$\mathcal{C}$表示正交坐标系$O;\vec{\imath};\vec{\jmath}$的函数$h$的曲线。
}
{{%
\begin{minipage}{5cm}
