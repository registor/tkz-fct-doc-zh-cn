...
对第1个函数命名为\tkzcname{tkzFcta}，第2个函数命名\tkzcname{tkzFctb}，以此类推\ldots。
如果一个环境中有多个函数，则可以使用\tkzname{with}指定需要使用的函数。

\tkzHandBomb 注意在使用\tkzcname{tkzFct}命令和\tkzcname{tkzDrawTangentLine}命令之前，
...
其他情况下，如果半切线向量$\vec{AT}$的斜率是$p$，则半切线的坐标是(\tkzname{kl},\tkzname{kl*p})。


% \subsection{Représentation d'une tangente \tkzcname{tkzDrawTangentLine}}
\subsection{\tkzcname{tkzDrawTangentLine}命令：绘制函数切线}
\hypertarget{tdtl}{}
...
\begin{NewMacroBox}{tkzDrawTangentLine}{\oarg{命令选项}\parg{a}}
\emph{
如果直接在\tkzcname{tkzFct}命令后使用该命令，则表示使用当前函数。
否则，需要用\tkzname{with}指定函数。}
...
如果\tkzname{kl}或\tkzname{kr}为0，则只绘制半切线，否则，可以用数字表示切线长度与原始切线长度的百分比。
...
可以将\tkzcname{tkzDrawTangentLine}放在第一个函数定义代码行之后，表示使用该函数$(a)$，
...
在此，只需得到0和1之间的半切线，仅需为\tkzname{kr}和\tkzname{kl}赋合适的值即可。
...
为此，必须使用符合\tkzname{fp.sty}宏包语法的表达式定义函数。

需要定义\tkzcname{tkzFctLast}全局宏。
...
如\tkzname{(\{1/exp(1)\})}是正确的，但\tkzname{(1/exp(1))}则会引发错误。
在后续计算中，也需要注意该问题。

\subsubsection{提前定义计算结果}
